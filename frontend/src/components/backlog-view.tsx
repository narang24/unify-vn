...
import { WORK_ITEM_TYPES, type WorkItemType } from "@/lib/work-item-types";
...
function BacklogSection({
  ...
  return (
    <div className="rounded-2xl border border-border-subtle bg-panel">
      <div className="flex items-center gap-2 border-b border-border-subtle px-3 py-2.5">
        <button onClick={onToggle} className="rounded-md p-0.5 hover:bg-black/5" aria-label="Toggle section">
      ...
      {items.map((item) => {
        ...
        return (
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex items-center gap-2 rounded-lg border border-border-subtle px-2.5 py-2 hover:bg-black/[0.03]"
          >
...
function CreateRow({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="mt-2 flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[12.5px] text-muted hover:bg-black/5 hover:text-foreground"
    >
